...
from documentor.docs.data import DocumentData
...
def test_document() -> None:
    ...
    doc.file_path = get_path('data/microsoft-annual-report.pdf')
    ...
    doc.cache_dir_path = get_path('cache2')
    print(f"CacheMetadata = {json.dumps(doc.cache_metadata(), indent=4)}")
    
    # change the cache filepath and the filetype also changes
    doc.cache_file_path = get_path('cache/cache_file.csv')
    print(f"CacheMetadata = {json.dumps(doc.cache_metadata(), indent=4)}")\
